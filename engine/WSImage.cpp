		...
		for (uint16_t y = 0; y < this->m_height; y++) {
			...
			for (uint16_t x = 0; x < this->m_width; x++) {
				...
				case 1: // Sub (LEFT)
					...
					pixelColor += this->m_pBuff[i - 1u];
					break;
				case 2: // Up
					if (y == 0) break;

					pixelColor += this->m_pBuff[i - this->m_width];
					break;
				case 3: // Average
					if (y == 0 || x == 0) break;

					{
						...
						pixelColor.r += static_cast<uint8_t>(std::floor((static_cast<float>(topPixel.r)+static_cast<float>(leftPixel.r)) / 2.f));
						pixelColor.g += static_cast<uint8_t>(std::floor((static_cast<float>(topPixel.g)+static_cast<float>(leftPixel.g)) / 2.f));
						pixelColor.b += static_cast<uint8_t>(std::floor((static_cast<float>(topPixel.b)+static_cast<float>(leftPixel.b)) / 2.f));
						pixelColor.a += static_cast<uint8_t>(std::floor((static_cast<float>(topPixel.a)+static_cast<float>(leftPixel.a)) / 2.f));
					}
